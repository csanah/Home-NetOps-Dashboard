...
def _fetch_library_counts(key):
    ...
    try:
        ...
        logger.debug("Failed to fetch unwatched count for library %s: %s", key, e)
        ...
        with ThreadPoolExecutor(max_workers=len(sections) or 1) as pool:
            ...
            for future in as_completed(future_to_idx, timeout=10):
                ...
                try:
                    ...
                    logger.debug("Failed to get library counts for section %s: %s", idx, e)
...
def _fetch_history():
    ...
    try:
        ...
        try:
            ...
            logger.debug("Failed to fetch bandwidth stats for account names: %s", e)
...
def auto_detect_plex_token():
    ...
    for path in search_paths:
        try:
            ...
            logger.debug("Failed to read Preferences.xml at %s: %s", path, e)
            continue
    ...
    if not token:
        try:
            ...
            logger.debug("Failed to find Preferences.xml via search: %s", e)
