...
def get_health():
    ...
    try:
        ...
        r = _session.get(f"{base}/nodes/{node}/status", headers=headers, verify=False, timeout=8)
        r.raise_for_status()
        d = r.json().get("data", {})
        info["online"] = True
    ...
    # VMs
    try:
        r = _session.get(f"{base}/nodes/{node}/qemu", headers=headers, verify=False, timeout=8)
        r.raise_for_status()
    ...
    # LXC containers
    try:
        r = _session.get(f"{base}/nodes/{node}/lxc", headers=headers, verify=False, timeout=8)
        r.raise_for_status()
