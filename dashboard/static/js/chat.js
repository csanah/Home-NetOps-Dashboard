...
function renderAssistantBubble() {
  ...
  currentAssistantBubble.innerHTML = DOMPurify.sanitize(marked.parse(currentResponseText));
  scrollToBottom();
}
...
function addCopyButtons(container) {
  // Only scan the given container element (not the entire DOM)
  const pres = container.tagName === 'PRE' ? [container] : container.querySelectorAll('pre');
  pres.forEach((pre) => {
...
// ── Generation Timeout (2 minutes of no chunks) ──
let generationTimeout = null;
const GENERATION_TIMEOUT_MS = 120000; // 2 minutes

function resetGenerationTimeout() {
  if (generationTimeout) clearTimeout(generationTimeout);
  if (!isGenerating) return;
  generationTimeout = setTimeout(() => {
    if (isGenerating) {
      appendErrorBubble('No response received for 2 minutes — generation may have stalled.');
      finishGeneration();
    }
  }, GENERATION_TIMEOUT_MS);
}

// Patch output handler to reset timeout on each chunk
(function() {
  const origOutputHandler = chatSocket.listeners('output')[0];
  if (origOutputHandler) {
    chatSocket.off('output', origOutputHandler);
    chatSocket.on('output', function(data) {
      resetGenerationTimeout();
      origOutputHandler(data);
    });
  }
})();

// Patch generation_started to start timeout
(function() {
  const origStartedHandler = chatSocket.listeners('generation_started')[0];
  if (origStartedHandler) {
    chatSocket.off('generation_started', origStartedHandler);
    chatSocket.on('generation_started', function() {
      origStartedHandler();
      resetGenerationTimeout();
    });
  }
...
(function() {
  const origFinish = finishGeneration;
...
document.getElementById('btn-copy-chat').addEventListener('click', function() { copyChatHistory(); });
document.getElementById('btn-clear').addEventListener('click', function() { clearHistory(); });
document.getElementById('btn-send').addEventListener('click', function() { sendMessage(); });
document.getElementById('btn-stop').addEventListener('click', function() { cancelGeneration(); });
